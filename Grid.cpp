...
HRESULT Grid::Init(ID3D11Device * pDevice, int size)
{
	...
	for (int i = 0; i < totalV; i += 4)
	{
		if (cell - a == 0)
		{
			v[i + 0] = { XMFLOAT3(cell - a, 0, cell), XMFLOAT4(0,0,1,1) };
			v[i + 1] = { XMFLOAT3(cell - a, 0, -cell), XMFLOAT4(0,0,1,1) };
			v[i + 2] = { XMFLOAT3(cell, 0, cell - a), XMFLOAT4(1,0,0,1) };
			v[i + 3] = { XMFLOAT3(-cell, 0, cell - a), XMFLOAT4(1,0,0,1) };
			a++;
			continue;
		}
	...
	v[totalV + 0] = { XMFLOAT3(0,cell,0), XMFLOAT4(0,1,0,1) };
	v[totalV + 1] = { XMFLOAT3(0,-cell,0), XMFLOAT4(0,1,0,1) };

	D3D11_BUFFER_DESC vbd;
	vbd.Usage = D3D11_USAGE_IMMUTABLE;
	vbd.ByteWidth = sizeof(PC) * totalV + sizeof(PC) * 2;
	...
	UINT *ii = new UINT[totalV + 2];
	for (int i = 0; i < totalV + 2; i++)
	{
		ii[i] = i;
	}
	D3D11_BUFFER_DESC ibd;
	ibd.Usage = D3D11_USAGE_IMMUTABLE;
	ibd.ByteWidth = sizeof(UINT) * totalV + sizeof(UINT) * 2;
